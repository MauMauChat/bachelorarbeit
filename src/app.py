...
class MainWindow:
    ...
    def run_analysis(self):
        ...
        try:
            ...
            # Alle ausgewählten Verzeichnisse verarbeiten
            for directory in self.selected_directories:
                analyzer.search_and_process(directory)

            # Nach der Sentimentanalyse das R-Skript ausführen
            self.log_output("Generiere den Bericht mit R...")
            # Wir übergeben das Hauptverzeichnis, um alle Ergebnisse zu sammeln
            root_directory = os.path.commonpath(self.selected_directories)
            self.run_r_script(root_directory)
    ...
    def run_r_script(self, directory):
        ...
        # Pfade an das R-Skript übergeben
        try:
            subprocess.run(
